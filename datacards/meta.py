...
def parse_missing_attributes(error: TypeError):
    """Find all arguments mentioned not initialized."""
    err = str(error)
    return re.findall("'(.*?)'", err)


class YamlParsingError(TypeError):
    def __init__(
        self, message="No yaml table was found when parsing the dataset README"
    ):
        self.message = message
        super().__init__(self.message)


class YamlDuplicateKeysError(TypeError):
    def __init__(self, message="Got duplicate yaml keys"):
        self.message = message
        super().__init__(self.message)


class YamlMultipleConfigsError(TypeError):
    def __init__(self, message="Found multiple conflicting config files"):
        self.message = message
        super().__init__(self.message)


class MetaDataValidationError(TypeError):
    def __init__(self, message="Could not parse or validate the dataset attributes"):
        self.message = message
        super().__init__(self.message)
...
class NoDuplicateSafeLoader(yaml.SafeLoader):
    def _check_no_duplicates_on_constructed_node(self, node):
        ...
        if duplicate_keys:
            raise YamlDuplicateKeysError(f"Got duplicate yaml keys: {duplicate_keys}")
...
def validate_metadata_type(metadata_dict: dict):
    ...
    if len(typing_errors) > 0:
        raise MetaDataValidationError(
            f"The following typing errors are found: {typing_errors}"
        )
...
class CustomDatasetMetadata:
    ...
    annotations_creators: Union[List[str], Dict[str, List[str]]] = field(default=None)
    language_creators: Union[List[str], Dict[str, List[str]]] = field(default=None)
    languages: Union[List[str], Dict[str, List[str]]] = field(default=None)
    licenses: Union[List[str], Dict[str, List[str]]] = field(default=None)
    multilinguality: Union[List[str], Dict[str, List[str]]] = field(default=None)
    pretty_name: Union[str, Dict[str, str]] = field(default=None)
    size_categories: Union[List[str], Dict[str, List[str]]] = field(default=None)
    source_datasets: Union[List[str], Dict[str, List[str]]] = field(default=None)
    task_categories: Union[List[str], Dict[str, List[str]]] = field(default=None)
    task_ids: Union[List[str], Dict[str, List[str]]] = field(default=None)
    paperswithcode_id: Optional[str] = field(default=None)
    # Custom
    extended: Union[List[str], Dict[str, List[str]]] = field(default=None)
    ...
    def from_readme(cls, path: Path) -> "DatasetMetadata":
        ...
        if yaml_string is not None:
            ...
            raise YamlParsingError(f"Unable to find a yaml block in '{path}'")
...
class DatasetMetadata:
    ...
    def validate_pretty_name(pretty_name: Union[str, Dict[str, str]]):
        if isinstance(pretty_name, str):
            if len(pretty_name) == 0:
                return (
                    None,
                    "The pretty name must have a length greater than 0 but got an empty string.",
                )
        else:
            error_string = ""
    ...
    def get_metadata_by_config_name(self, name: str) -> "DatasetMetadata":
        ...
        if len(has_multi_configs) > 0 and has_multi_configs != config_name_hit:
            raise YamlMultipleConfigsError(
